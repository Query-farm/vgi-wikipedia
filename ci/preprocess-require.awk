...
# A bare `INSTALL json;` (the json extension the `extra->>` extraction needs) must
# resolve from the signed core channel on a clean runner. Pin its source.
/^[ \t]*INSTALL[ \t]+(json|httpfs|parquet|spatial)[ \t]*;[ \t]*$/ {
    n = $2; sub(/;.*/, "", n)
    print "INSTALL " n " FROM core;"; next
}
{ print }
